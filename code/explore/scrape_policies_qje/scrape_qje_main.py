"""
Script to scrape policy-related papers from the Quarterly Journal of Economics (QJE).
...
# QJE Source ID in OpenAlex
QJE_SOURCE_ID = "S203860005"
...
def search_qje(query, per_page=100, max_results=500):
    """
    Search OpenAlex for papers matching the query within QJE only.
    ...
    print(f"  Searching QJE for: '{query}'")

    while len(all_results) < max_results:
        params = {
            'search': query,
            'filter': f'primary_location.source.id:{QJE_SOURCE_ID}',
            'per_page': per_page,
            'page': page,
            'mailto': USER_EMAIL
        }
...
def process_policy(policy_row):
    ...
    print(f"\n{'='*80}")
    print(f"Processing: {policy_name} ({policy_abbr}) in QJE")
    ...
    if len(df) == 0:
        print(f"\n  WARNING: No QJE papers found for {policy_name}")
        return {
    ...
    parquet_file = os.path.join(OUTPUT_DIR, f"{policy_abbr}_papers_qje.parquet")
    df_unique.to_parquet(parquet_file, index=False, engine='pyarrow')
    print(f"\n  Saved Parquet: {parquet_file}")

    csv_file = os.path.join(OUTPUT_DIR, f"{policy_abbr}_papers_qje.csv")
    ...
    metadata = {
        ...
        'journal_source_id': QJE_SOURCE_ID,
    ...
    if len(df_unique) > 0:
        print(f"\n  SUMMARY for {policy_abbr} in QJE:")
...
def main():
    ...
    """
    print("="*80)
    print("POLICY PAPERS SCRAPING FROM QJE (via OpenAlex)")
    print("="*80)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"QJE Source ID: {QJE_SOURCE_ID}")
    print()
    ...
    print(f"\n{'='*80}")
    print("OVERALL SUMMARY - QJE")
    print(f"{'='*80}")

    summary_df = pd.DataFrame(all_summaries)
    print(summary_df.to_string(index=False))

    summary_file = os.path.join(OUTPUT_DIR, "all_policies_qje_summary.csv")
